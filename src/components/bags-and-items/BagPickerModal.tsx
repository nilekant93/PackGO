        ...
        {availableBags.length === 0 ? (
          <View style={{ paddingVertical: 18 }}>
            ...
            <View style={{ marginTop: 16 }}>
              <GradientButton
                label="Manage your bags"
                onPress={() => {
                  onClose();
                  router.push({ pathname: "/your-stuff", params: { step: "bags" } });
                }}
              />
            </View>
          </View>
            ...
            renderItem={({ item: bag }) => (
              <Pressable onPress={() => onSelectBag(bag)} style={styles.modalBagRow}>
                ...
                <View style={{ flexShrink: 1 }}>
                  <Text style={{ color: "#E2E8F0", fontWeight: "900" }} numberOfLines={1}>
                    {bag.name}
                  </Text>
                  <Text style={{ color: "#94A3B8", marginTop: 2, fontSize: 12 }} numberOfLines={1}>
                    {bag.type}
                  </Text>
                </View>
              </Pressable>
